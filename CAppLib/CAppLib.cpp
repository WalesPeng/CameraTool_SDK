...
static int convert_yuv_to_rgb_pixel(int y, int u, int v)
{
		...
		//*************************************YUV转RGB修改***********************************************
		// YKB 20180503 优化YUV转RGB，速度提升不大（就只提升了2ms左右，为啥位移比浮点只快这么点？不科学？？）

		//b = (y << 8) + 351 * (v - 128); // 全部浮点
		//g = (y << 8) - 179 * (v - 128) - 86 * (u - 128);
		//r = (y << 8) + 444 * (u - 128);
		//b = b < 0 ? 0 : (b >> 8);
		//g = b < 0 ? 0 : (g >> 8);
		//r = b < 0 ? 0 : (r >> 8);
		//b = ((y << 8) + 351 * v - 44928) >> 8; // 全部浮点
		//g = ((y << 8) - 179 * v - 86 * u + 33920) >> 8;
		//r = ((y << 8) + 444 * u - 56832) >> 8;
		//b = b < 0 ? 0 : (b >> 8);
		//g = b < 0 ? 0 : (g >> 8);
		//r = b < 0 ? 0 : (r >> 8);
		//b = (y << 8) + (v << 8) + (v << 6) + (v << 4) + (v << 3) + (v << 2) + (v << 1) + v - 44928; // 全部浮点
		//g = (y << 8) - ((v << 7) + (v << 5) + (v << 1) + v) - ((u << 6) + (u << 4) + (u << 2) + (u << 1)) + 33920;
		//r = (y << 8) + ((u << 8) + (u << 7) + (u << 5) + (u << 4) + (u << 3) + (u << 2)) - 56832;
		//b = b < 0 ? 0 : (b >> 8);
		//g = b < 0 ? 0 : (g >> 8);
		//r = b < 0 ? 0 : (r >> 8);
		//*************************************YUV转RGB修改结束***********************************************
